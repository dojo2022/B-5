...
<div class="wrapper">
  ...
  <div class="maincontents">
  	<div class="leftside">
		...
		<p>予定・ToDoリストの編集<br>日記の記入ができるよ～<br>日記を記入してポイントをゲットしよう！</p>
		</div>
  ...
  <div class="rightside">
  	<div class="edit">
    	...
    	<div class="scheduleedit">
            ...
            <details>
				<summary><select id="stampselect" name ="stamp_id" >
					<option value="1" ${e.stamp_id == '1' ? 'selected' : ''}>🍚</option>
				 	<option value="2" ${e.stamp_id == '2' ? 'selected' : ''}>💛</option>
				 	<option value="3" ${e.stamp_id == '3' ? 'selected' : ''}>⛰</option></select>
					<input type="text" class="titlefont" name="title" placeholder="タイトルを入力" value="${e.title}">
				</summary>
                <table>
	                  <tr>
                  ...
                  <div class="inpur_wrapper">
                 	...
                 	<input class="hover" type="submit" name="DELETE" value="削除" onsubmit="doSomething();return false;">
                  	<input class="hover" type="submit" name="UPDATE" value="保存" onsubmit="doSomething();return false;">
                  </div>
            </details>
            </form>
            </c:forEach>
         ...
         <div class="todoedit">
	         ...
	         <form method="POST" action="/anikare/ScheduleEditServlet">
		         <details>
		             <summary>
		             	<input type="checkbox" id="checlselect" name="checkbox" value="missioncomplete">
		             	<input type="text" class="titlefont" name="task" placeholder="タスク名を入力" value="${e.task}">
		             </summary>
		             <table>
                        <tr>
                          <td><input type="text" class="shorttext" name="todo_deadline" placeholder="締め切り時間を入力" value="${e.todo_deadline}"></td>
                        </tr>
                        <tr><td>
                     ...
                     <div class="input_wrapper">
	                    <a href="/anikare/TopPageServlet">戻る</a>
	                    <input class="hover" type="submit" name="DELETE" value="削除">
	                 	<input class="hover" type="submit" name="UPDATE" value="保存">
                  	 </div>
                  </details>
               </form>
               </c:forEach>
      ...
      <div class="diaryedit">
         ...
         <form method="POST" action="/anikare/ScheduleEditServlet">
	          <table>
	            <tr>
	              <td><input type="text" class="titlefont" name="diary_title" placeholder="タイトルを入力" value="${diaryList[0].diary_title}"></td>
	            </tr>
	            <tr>
	              <td><input type="text" class="diarycontent" name="diary_content" value="${diaryList[0].diary_content}"></td>
	              <td><input type="hidden" name="diary_date" value="${diaryList[0].diary_date}"></td>
	              <td><input type="hidden" name="user_id" value="${id.user_id}"></td>
	            </tr>
	          </table>
            <div class="input_wrapper">
            	<input class="hover" type="submit" name="DELETE" value="リセット">
           		<input class="hover" type="submit" name="UPDATE" value="保存">
           	</div>
        </form>
